# StartNode: A Neuro-Graphic Self-Learnable Engine for Cognitive GUI Automation


## What is StartNode?

StartNode is a self-operating computer system designed to automate web interactions and data extraction processes. It leverages advanced technologies like OCR (Optical Character Recognition), YOLO (You Only Look Once) models for object detection, and a custom site-graph to navigate and interact with web pages programmatically.


To get started with StartNode, you need to have Python installed on your system. Follow these steps to install StartNode:

1. Open your terminal and clone the SuperAGI repository.
```
git clone https://github.com/KhulnaSoft/StartNode.git 
```
...
cd StartNode
```
3. Create a copy of .env.example , and name it .env. Repeat this step for all the three modules - startnode, yolo, ocr
...
5. Once you have Docker Desktop running, run the following command in the StartNode directory:
...
## How to Use StartNode

StartNode operates based on a site-graph that defines the navigation and actions to be performed on a website. Here's a basic overview of how to use StartNode:

1. Define Your Objective: Specify what you want to achieve with StartNode, such as data extraction or automation of specific web interactions.


2. Prepare Your Autonode-Site-Graph: Create a JSON file that represents the site-graph. This graph outlines the nodes (web elements) and edges (actions) that StartNode will navigate and interact with.


3. Prepare you StartNode initiator planner prompt : Using the template structure given for planner prompts in openai_prompts.py, for OpenAI LLM, you can create a new prompt file in the prompts directory with structure <llm_prompts.py> 


4. Run StartNode: 

    ### Using StartNode via API

    StartNode can be controlled and utilized through its API, allowing users to automate web interactions and data extraction tasks programmatically. This guide will walk you through the process of sending requests to StartNode using its API endpoint.

    #### Accessing the API Documentation

    Before you start, ensure that StartNode is running on your local machine. Once StartNode is up and running, you can access the API documentation by visiting:

     ```
    http://localhost:8001/docs
     ```
     This URL will take you to the Swagger UI, where you can find detailed documentation on all available API endpoints, including the one used to initiate StartNode tasks.

     #### Sending a Request to StartNode

     To automate a task with StartNode, you will use the /api/startnode/initiate endpoint. This endpoint accepts a JSON payload that specifies the task's objective, the path to the site-graph JSON file, the root node to start traversal, and the URL of the website you wish to interact with.

     #### Request Structure
     Here is the structure of the JSON payload you need to send to the `/api/startnode/initiate` endpoint:
     ...
     Example request:
     {
        ...
        "graph_path": "startnode/site_trees/apollo.json"
        "planner_prompt": "apollo"
     }
     
     - site_url: The URL of the website StartNode will visit and interact with.
     ...
     - graph_path: The path to the JSON file that contains your site-graph. The site-graph defines the structure and navigation flow of the website for StartNode.

     - planner_prompt: The key for planning prompt. You can change or map new key in planning_agent.py for StartNode planner prompt.
   ...
   #### Using CURL
     ...
     ```
     curl -X 'POST' \
         'http://localhost:8001/api/startnode/initiate' \
...
StartNode utilizes YOLO models for object detection and OCR for text recognition on web pages. These models are crucial for identifying clickable elements, reading text from images, and interacting with web pages dynamically.
...
5. Integrate with StartNode: Once trained, integrate your custom YOLO model with StartNode by specifying the model path in the configuration.
...
### StartNode-Site-Graph Preparation
The site-graph is a JSON file that describes the structure and navigation flow of a website for StartNode. Here's how to prepare it:
...
1. Storing Screenshots & Downloads Remotely
 - In your startnode/.env file, configure AWS keys AWS_ACCESS_KEY_ID and AWS_SECRET_ACCESS_KEY from your AWS console
 - In startnode/services/startnode.py, uncomment the following snippet at line 35, 
...
```
 ...
 ```
    s3_client=None
 ```
 - In startnode/utils/screenshot_generator.py uncomment this on line 18,

 ```
    # Uncomment If you have aws account and want to store result in your AWS S3
    s3_client.upload_file(file_path=screenshot_filename)
 ```
 - In startnode/nodes/download.py uncomment this on line 44
 ...
 - In startnode/worker.py comment this finally block on line 79 to persist screenshots locally,
 ```
    finally:
