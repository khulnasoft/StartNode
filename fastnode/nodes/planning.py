...
from fastnode.agents.planning_agent import PlanningAgent
from fastnode.config.config import get_config
from fastnode.llms.open_ai import OpenAi
from fastnode.nodes.base_node import Node
from fastnode.factories.node_factory import NodeFactory
from fastnode.utils.exceptions.llm_objective_exception import LLMObjectiveException
from fastnode.utils.helpers.prompt_helper import format_planning_prompt
...
class PlanningNode(Node):
    ...
    def run(
        ...
        **kwargs
        ...
        Args:
            fastnode_object: Object representing the fastnode.
        ...
        fastnode_object.objective = format_planning_prompt(planning_agent.execute(objective=objective))
        logger.info(f"Objective for the request_id: {request_id} is: {fastnode_object.objective}")

        if fastnode_object.objective is None:
