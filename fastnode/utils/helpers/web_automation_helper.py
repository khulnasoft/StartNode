...
class WebAutomationHelper:
    ...
    @staticmethod
    def find_screenshot_size(path: str):
        img = Image.open(path)
        return img.size

    @staticmethod
    async def click_on_page(page, location: list, screenshot_size: tuple, click_count):
        if click_count == 1:
    ...
    @staticmethod
    async def type_str_on_page(page, text: str, system: str):
        if system == "Linux":
    ...
    @staticmethod
    async def type_multiple_elements_on_page(page, text_list: list, system: str, separation_key: str = "Enter"):
        if system == "Linux":
            await page.keyboard.type("FastNode", delay=100)
            await page.keyboard.down('Control')
            await page.keyboard.press('A')
