...
from fastnode.services.fastnode import FastnodeService
...
def initiate_fastnode(
    ...
    logger.info(f"Initiating Fastnode for request {request_id}")
    try:
        driver = FastnodeService(
        ...
        logger.error(f"Error while running fastnode for request_id: {request_id}: {str(e)}")
